		...
		<>
			{fetching ? (
				...
				<>
					<div className='profile-categories'>
						<div className='category-form-container'>
							...
							<form className='category-form' onSubmit={onAddCategory}>
								<div>
									<label htmlFor='add-category' className='col-form-label'>
										Add category
									</label>
								</div>
								<div>
									<select className='form-control text-input' value={newCategoryType} onChange={onChangeNewCategoryType}>
										<option value='income'>income</option>
										<option value='expense'>expense</option>
									</select>
								</div>
								<div>
									<Tippy content={categoryOverloadError || categoryInputError} visible={(categoryInputError || categoryOverloadError) && !showModal}>
										<input
					...
					<Modal aria-labelledby='contained-modal-title-vcenter' centered show={showModal} style={{ fontFamily: 'Arial', borderRadius: '50px' }}>
						<Modal.Header>
							<Modal.Title id='contained-modal-title-vcenter'>Are you sure you want to delete category {categoryOnDeletion.category}?</Modal.Title>
						</Modal.Header>
						<Modal.Body>
							<p>All transactions with this category will be moved to the "Other"-category.</p>
						</Modal.Body>
						<Modal.Footer>
							<button className='btn btn-primary' style={{ background: '#2e20ec' }} onClick={() => setShowModal(false)}>
								Cancel
							</button>
