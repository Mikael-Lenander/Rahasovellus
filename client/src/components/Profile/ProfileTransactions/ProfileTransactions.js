...
export default function ProfileTransactions() {
    ...
    categoryTypes = {income: ['Any', ...categoryTypes.income], expense: ['Any', ...categoryTypes.expense]}
    ...
    return (
        <div className='profile-section profile-transactions'>
                ...
                <form>
                    <div className='search-form'>
                        <div className='search-form-item'>
                            <label htmlFor='min-amount' style={{margin: 0}}>Min amount</label>
                            <input id='min-amount' type='number' className='form-control text-input' min='0' step='0.01'
                            value={minAmount} onChange={e => setMinAmount(e.target.value)}/>
                        </div>
                        <div className='search-form-item'>
                            <label htmlFor='max-amount' style={{margin: 0}}>Max amount</label>
                            <input id='max-amount' type='number' className='form-control text-input' min={minAmount.toString()} step='0.01'
                            value={maxAmount} onChange={e => setMaxAmount(e.target.value)}/>
                        </div>
                        <div className='search-form-item'>
                            <label htmlFor='from' style={{margin: 0}}>From</label>
                            <DatePicker className='form-control text-input' selected={minDate} onChange={date => setMinDate(date)}
                            dateFormat="dd/MM/yyyy" maxDate={maxDate}/>
                        </div>
                        <div className='search-form-item'>
                            <label htmlFor='to' style={{margin: 0}}>To</label>
                            <DatePicker className='form-control text-input' selected={maxDate} onChange={date => setMaxDate(date)}
                            dateFormat="dd/MM/yyyy" minDate={minDate}/>
                        </div>
                        <div className='search-form-item'>
                            <label htmlFor='type' style={{margin: 0}}>Type</label>
            ...
            <TransactionSearchTable transactions={search({
