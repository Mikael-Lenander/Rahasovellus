...
export default function NetWorthChart({dataset, onSetSvgHeight}) {
    ...
    useEffect(() => {
        if (dimensions && dataset.length > 0) {
            ...
            const yAxis = d3.axisLeft().scale(yScale).ticks(Math.floor(height / 50))
            const xAxis = d3.axisBottom().scale(xScale).ticks(Math.floor(width / 100))
            ...
            svg.select('#positive-networth-area')
                .attr('d', posArea(dataset))
            svg.select('#negative-networth-area')
                .attr('d', negArea(dataset))

            const yAxisTickSize = 6 + width / 80
            svg.select('.y-axis')
                .attr('transform', `translate(${margin.left}, 0)`)
                .style('font-size', `${yAxisTickSize}px`)
                .call(yAxis)
    ...
    return (
        <div ref={containerRef} style={{width: '100%', height: '100%', position: 'relative'}}>
            ...
            {dataset.length < 2 && <div className='flex-center blank-page'>
