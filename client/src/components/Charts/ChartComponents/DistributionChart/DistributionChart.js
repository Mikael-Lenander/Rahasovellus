...
export default function DistributionChart({dataset, type, onSetSvgHeight}) {
    ...
    useEffect(() => {
        if (dimensions && dataset.length > 0) {
            ...
            const keys = Object.keys(dataset[dataset.length - 1])
                .filter(key => key !== 'date' && dataset[dataset.length - 1][key] > 0)
            const legendSize = 18
            ...
            const series = d3.stack()
                .keys(keys)
                .offset(d3.stackOffsetExpand)(dataset)
            ...
            const colorScale = d3.scaleOrdinal()
                .domain(keys)
                .range(d3.schemeCategory10.slice(0, keys.length))
            ...
            svg.selectAll(`.${type}-distribution-area`)
            .data(series)
            .join("path")
