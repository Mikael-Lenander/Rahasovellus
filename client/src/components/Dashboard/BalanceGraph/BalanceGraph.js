...
import transactionCalculator from '../../../model/transactionCalculator'
...
export default function BalanceGraph() {
    ...
    useEffect(() => {
        if (dimensions && dataset.length > 0) {
            ...
            const positiveYScale = d3.scaleLinear()
            .domain([0, maxValue])
            .range([0, positiveBarRange])

            const negativeYScale = d3.scaleLinear()
            .domain([0, minValue])
            .range([0, negativeBarRange])
    ...
    return (
            <div id='balance-graph-container' className='dashboard-item balance-graph' ref={containerRef} style={{position: 'relative', zIndex: 1}}>
              {fetching && <LoadingScreen style={{color: 'black', width: 60, height: 60, position: 'absolute', top: '50%', marginTop: -30}} classes='flex-center'/>}
              <svg width='100%' height='100%' ref={svgRef}>
                <g className="x-axis" />
                <g className="y-axis" />
                <text className='title'></text>
              </svg>
              {(!fetching && Object.keys(dataset).length === 0) && <div className='flex-center blank-page'>
