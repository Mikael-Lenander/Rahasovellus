...
export default function Login() {
    ...
    return (
        <>
        <Header homeLink={{onClick: clearMessages}} links={[{path: '/login', text: 'Login'}, {path: '/register', text: 'Register', onClick: clearMessages}]}/>
        <main className='background flex-center'>
            <form className='credential-form'>
                    ...
                    <div className='form-margin'>
                        <button className='btn btn-danger btn-block' type='submit' onClick={onLogin}>Login</button>
                    </div>
                    <p className='form-margin'>
                        No account? <Link onClick={clearMessages} to='/register'>Register for free </Link>
                    </p>
                    {registerMessages.messages.map(message => <div className="alert alert-success form-margin"
                     role="alert" style={{marginTop: 10, marginBottom: 10}} key={message}>{message}</div>)}   
                    {loginMessage.message && <div className={`alert alert-${loginMessageStyle} form-margin`} role="alert"
                     style={{marginTop: 10, marginBottom: 10}}>{loginMessage.message}</div>}
            </form>
        </main>
        </>
    )
