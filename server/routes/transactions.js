...
router.post('/new', async (req, res) => {
    ...
    try {
        ...
        if (new Date(date) < new Date(oldestTransactionDate)) {
            await User.findByIdAndUpdate(_id, {$set: {oldestTransactionDate: new Date(date)}}, {useFindAndModify: false})
...
router.delete('/delete/:id', async (req, res) => {
    ...
    try {
        ...
        if (dayjs(oldestTransactionDate).isSame(dayjs(deletedTransaction.date))) {
            ...
            await User.findByIdAndUpdate(userId, {$set: {oldestTransactionDate: new Date(oldestTransactionDate)}},
             {useFindAndModify: false})
        }
        console.log('Transaction deleted')
        res.send({transaction: deletedTransaction._id, oldestTransactionDate: oldestTransactionDate})
    }
    catch (error) {
        console.log(error)
    }
