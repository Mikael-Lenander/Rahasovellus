...
router.put('/initCapital', (req, res) => {
    ...
    const errorMessage = {message: 'Updating initial capital failed', success: false}
    User.findById(userId, (err, user) => {
        ...
        user.save((err, user) => {
            ...
            res.json({message: 'Initial capital updated', success: true, data: user.initCapital})
        })
    })
})
...
router.put('/password', async (req, res) => {
    ...
    try {
        ...
        if (match) return res.json({message: 'This is your current password', success: false})
    }
    catch(error) {
        console.log(error)
        return res.json(errorMessage)
    }
    if (password.length < 6) return res.json({message: 'Password should be at least 6 characters long', success: false})
...
router.delete('/category', async (req, res) => {
    ...
    try {
        ...
        res.json({categories: categories, transactionIds: updatedTransactions.map(transaction => transaction._id)})
    }
    catch(error) {
        console.log(error)
    }
